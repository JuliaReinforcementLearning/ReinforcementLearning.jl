...
function reset!(env::KuhnPokerEnv)
    empty!(env.cards)
    empty!(env.actions)
end
...
function reward(env::KuhnPokerEnv, p)
...
ActionStyle(::KuhnPokerEnv) = MINIMAL_ACTION_SET
InformationStyle(::KuhnPokerEnv) = IMPERFECT_INFORMATION
