...
mutable struct TDLearner{Tapp <: AbstractApproximator, method, O} <: AbstractLearner
    ...
    function TDLearner(;approximator::Tapp, γ=1.0, optimizer=Descent(1.0), n=0, method::Symbol=:SARSA) where {Tapp<:AbstractApproximator}
...
function extract_transitions(buffer::EpisodeTurnBuffer, learner::TDLearner{<:AbstractVApproximator, :SRS})
    n = learner.n
    if length(buffer) > 0
        @views (
...
function extract_transitions(buffer::EpisodeTurnBuffer, learner::TDLearner{<:AbstractQApproximator, :SARSA})
    n = learner.n
    if length(buffer) > 0
        @views (
            states=state(buffer)[max(1, end - n - 1):end-1],
