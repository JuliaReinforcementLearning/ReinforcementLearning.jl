...
function quantile_huber_loss(ŷ, y; κ=1.0f0)
    ...
    cum_prob = ignore_derivatives() do
        send_to_device(KernelAbstractions.get_backend(y), range(0.5f0 / N; length=N, step=1.0f0 / N))
    end
...
function RLCore.optimise!(learner::QRDQNLearner, ::PostActStage, trajectory::Trajectory)
