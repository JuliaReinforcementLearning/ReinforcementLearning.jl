...
(learner::BasicDQNLearner)(obs) = obs |>
    get_state |>
...
function BasicDQNLearner(
    ...
    BasicDQNLearner{Q, F, typeof(rng)}(approximator, loss_func, γ, batch_size, min_replay_history, rng)
end

function RLBase.update!(learner::BasicDQNLearner, batch)
    Q, γ, loss_func, batch_size = learner.approximator, learner.γ, learner.loss_func, learner.batch_size
    s, r, t, s′ = map(x->send_to_device(device(Q), x), (batch.state, batch.reward, batch.terminal, batch.next_state))
...
function RLBase.extract_experience(
    t::CircularCompactSARTSATrajectory, learner::BasicDQNLearner
)
