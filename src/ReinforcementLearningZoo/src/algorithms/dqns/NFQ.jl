...
function RLBase.optimise!(learner::NFQ, ::PostEpisodeStage, trajectory::Trajectory)
    ...
    for i = 1:learner.num_iterations
        ...
        G = r .+ γ .* (cat(repeat(s′, inner=(1, 1, las)), reshape(repeat(as, outer=(1, size(s′, 2))), (1, size(s′, 2), las)), dims=1) |> x -> maximum(RLCore.forward(Q, x), dims=3) |> vec)
        for _ = 1:learner.epochs
            Flux.train!((x, y) -> loss_func(RLCore.forward(Q, x), y), Q.model, [(vcat(s, a), transpose(G))], Q.optimiser_state)
        end
    end
end
