...
function RLCore.forward(learner::NFQ, env::AbstractEnv)
    as = action_space(env)
    return vcat(repeat(state(env), inner=(1, length(as))), transpose(as)) |> x -> send_to_device(KernelAbstractions.get_backend(learner.approximator), x) |> x->RLCore.forward(learner, x) |> send_to_host |> vec 
end
...
function RLBase.optimise!(learner::NFQ, ::PostEpisodeStage, trajectory::Trajectory)
    ...
    a = Float32.(a)
    s, a, r, ss = map(x->send_to_device(KernelAbstractions.get_backend(Q), x), (s, a, r, ss))
