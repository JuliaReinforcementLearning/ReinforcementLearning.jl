...
mutable struct DQNLearner{Tq<:AbstractApproximator, Tt<:AbstractApproximator, Tf, S<:Union{Int, Nothing}, R<:AbstractRNG} <: AbstractLearner
    approximator::Tq
...
function RLBase.extract_experience(t::AbstractTrajectory, learner::DQNLearner)
    ...
    if length(t) > learner.min_replay_history
        ...
        (states=states, actions=actions, rewards=rewards, terminals=terminals, next_states=next_states)
    else
        nothing
    end
end