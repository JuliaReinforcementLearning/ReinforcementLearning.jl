...
function RLBase.optimise!(learner::DQNLearner, batch::NamedTuple)
    ...
    Q = A.model.source
    Qₜ = A.model.target
    @assert RLCore.device(Q) == RLCore.device(Qₜ) || @warn "Q and target Q function have to be on the same device"
    ...
    s, s_next, a, r, t = send_to_device(RLCore.device(Q), (s, s_next, a, r, t))
