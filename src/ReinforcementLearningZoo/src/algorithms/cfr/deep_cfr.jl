...
function RLBase.prob(π::DeepCFR, env::AbstractEnv)
    I = send_to_device(KernelAbstractions.get_backend(π.Π), state(env))
    m = send_to_device(KernelAbstractions.get_backend(π.Π), ifelse.(legal_action_space_mask(env), 0.0f0, -Inf32))
...
function RLCore.update!(π::DeepCFR)
    ...
    Π_losses = π.Π_losses
    Π_norms = π.Π_norms
    D = KernelAbstractions.get_backend(Π)
    MΠ = π.MΠ
    ps = Flux.params(Π)
...
function update_advantage_networks(π, p)
    ...
    if length(MV) >= π.batch_size_V
        for i in 1:π.n_training_steps_V
            ...
            I = send_to_device(KernelAbstractions.get_backend(V), Flux.batch([MV[:I][i] for i in batch_inds]))
            r̃ = send_to_device(KernelAbstractions.get_backend(V), Flux.batch([MV[:r̃][i] for i in batch_inds]))
            t = send_to_device(KernelAbstractions.get_backend(V), Flux.batch([MV[:t][i] / π.t for i in batch_inds]))
            m = send_to_device(KernelAbstractions.get_backend(V), Flux.batch([MV[:m][i] for i in batch_inds]))
            ps = Flux.params(V)
            gs = gradient(ps) do
...
function external_sampling!(π::DeepCFR, env::AbstractEnv, p)
    ...
    elseif current_player(env) == p
        V = π.V[p]
        s = state(env)
        I = send_to_device(KernelAbstractions.get_backend(V), Flux.unsqueeze(s, dims=ndims(s) + 1))
        A = action_space(env)
