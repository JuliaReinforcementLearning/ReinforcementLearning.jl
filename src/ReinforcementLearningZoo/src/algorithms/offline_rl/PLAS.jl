...
function (l::PLASLearner)(env)
    s = send_to_device(KernelAbstractions.get_backend(l.policy), state(env))
...
function update_vae!(l::PLASLearner, batch::NamedTuple{SARTS})
    s, a, r, t, s′ = send_to_device(KernelAbstractions.get_backend(l.vae), batch)
...
function update_learner!(l::PLASLearner, batch::NamedTuple{SARTS})
    s, a, r, t, s′ = send_to_device(KernelAbstractions.get_backend(l.qnetwork1), batch)
