Base.@kwdef mutable struct CQLSACPolicy{P<:SACPolicy}
    sac::P
...
function conservative_loss(p::CQLSACPolicy, qnetwork, q_policy_inputs, logps, s, a)
    q_policy = vec(LogExpFunctions.logsumexp(qnetwork(q_policy_inputs) .- logps, dims = 2)) #(1 x 1 x batchsize) -> (batchsize,) Note: python public implementations use a temperature.

    q_beta = vec(qnetwork(vcat(s, a))) #(batchsize,)

    diff = mean(q_policy . - q_beta)*p.cons_weight - p.τ_cql #The 1f0 can be replaced by a scaling parameter

    if p.α_cql_autotune
        p.α_cql += p.α_lr*diff
        clamp!(p.α_cql, 0f0,1f6)
    end
...
function update_critic!(p::CQLSACPolicy, batch::NamedTuple{SS′ART})
    ...
    y = soft_q_learning_target(p.sac, r, t, s′, p.sac.α, p.sac.γ)

    states = MLUtils.unsqueeze(s, dims = 2) #(state_size x 1 x batchsize)
    a_policy, logp_policy = p.sac.policy(states, p.action_sample_size) #(action_size x action_sample_size x batchsize), (1 x action_sample_size x batchsize)
    
    next_states = MLUtils.unsqueeze(s′, dims = 2) #(state_size x 1 x batchsize)
    a_policy′, logp_policy′ = p.sac.policy(next_states, p.action_sample_size) #(action_size x action_sample_size x batchsize), (1 x action_sample_size x batchsize)
    ...
    repeated_states = reduce(hcat, Iterators.repeated(states, p.action_sample_size*3)) #(state_size x action_sample_size*3 x batchsize)
    actions = hcat(a_policy, a_policy′, a_unif) #(action_size x action_sample_size*3 x batchsize)
    
    q_policy_inputs = vcat(repeated_states, actions)
    logps = hcat(logp_policy, logp_policy′, logp_unif) #(1 x action_sample_size*3 x batchsize)
