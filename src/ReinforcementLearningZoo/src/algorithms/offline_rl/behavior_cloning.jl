...
function RLBase.plan!(p::BehaviorCloningPolicy, env::AbstractEnv)
    ...
    s_batch = send_to_device(KernelAbstractions.get_backend(p.approximator), s_batch)
...
function RLCore.update!(p::BehaviorCloningPolicy, batch::NamedTuple{(:state, :action)})
    s = send_to_device(KernelAbstractions.get_backend(p.approximator), batch.state)
    a = send_to_device(KernelAbstractions.get_backend(p.approximator), batch.action)
    m = p.approximator
...
function RLBase.prob(p::BehaviorCloningPolicy, env::AbstractEnv)
    s = state(env)
    m = p.approximator
    s_batch = send_to_device(KernelAbstractions.get_backend(m), Flux.unsqueeze(s, dims=ndims(s) + 1))
