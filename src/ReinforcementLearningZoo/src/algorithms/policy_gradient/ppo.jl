...
function RLBase.prob(
    ...
    if p.update_step < p.n_random_start
        ...
        μ, σ =
            p.approximator.actor(send_to_device(KernelAbstractions.get_backend(p.approximator), state)) |>
            send_to_host
...
function RLBase.prob(p::PPOPolicy{<:ActorCritic,Categorical}, state::AbstractArray, mask)
    logits = p.approximator.actor(send_to_device(KernelAbstractions.get_backend(p.approximator), state))
    if !isnothing(mask)
...
function _update!(p::PPOPolicy, t::Any)
    ...
    D = KernelAbstractions.get_backend(AC)
