...
function RLCore.forward(learner::MACLearner, env::MultiThreadEnv)
    learner.approximator.actor(send_to_device(KernelAbstractions.get_backend(learner.approximator), state(env))) |>
    send_to_host
end
...
function RLCore.forward(learner::MACLearner, env)
    ...
    s = send_to_device(KernelAbstractions.get_backend(learner.approximator), s)
