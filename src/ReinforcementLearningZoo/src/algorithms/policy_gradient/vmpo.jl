...
function RLBase.prob(policy::VMPOPolicy{<:ActorCritic,Categorical}, env::AbstractEnv)
    s = send_to_device(KernelAbstractions.get_backend(policy.approximator), state(env))
...
function RLBase.plan!(agent::Agent{<:VMPOPolicy{<:ActorCritic{<:GaussianNetwork},Normal}},
    ...
    s = send_to_device(KernelAbstractions.get_backend(agent.policy.approximator), state(env))
