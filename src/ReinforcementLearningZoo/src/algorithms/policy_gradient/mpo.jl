...
function update_critic!(p::MPOPolicy, batches)
    for (id, batch) in enumerate(batches)
        ...
        a′ = RLCore.estimate_reward(p.actor, p.rng, s′; is_sampling=true, is_return_log_prob=false)
        q′_input = vcat(s′, a′)
        q′ = min.(RLCore.estimate_reward(p.target_qnetwork1, q′_input), RLCore.estimate_reward(p.target_qnetwork2, q′_input))
        ...
        if id % 2 == 0
            q_grad_1 = gradient(Flux.params(p.qnetwork1)) do
                q1 = RLCore.estimate_reward(p.qnetwork1, q_input) |> vec
                l = mse(q1, y)
            ...
            q_grad_2 = gradient(Flux.params(p.qnetwork2)) do
                q2 = RLCore.estimate_reward(p.qnetwork2, q_input) |> vec
                l = mse(q2, y)
                ignore_derivatives() do 
...
function update_actor!(p::MPOPolicy, batches::Vector{<:NamedTuple{(:state,)}})
    ...
    current_action_dist_batches = [RLCore.estimate_reward(p.actor, p.rng, states, is_sampling = false) for states in states_batches] #π(.|s,Θᵢ) 
    ...
    for (states, current_action_dist, action_samples) in zip(states_batches, current_action_dist_batches, action_samples_batches)
        ...
        Q = RLCore.estimate_reward(p.qnetwork1, input) 
...
function mpo_loss(p::MPOPolicy{<:Approximator{<:CovGaussianNetwork}}, qij, states, actions, μ_L_old::Tuple)
    μ_old, L_old = μ_L_old
    μ, L = RLCore.estimate_reward(p.actor, p.rng, states, is_sampling = false)
    #decoupling
...
function mpo_loss(p::MPOPolicy{<:Approximator{<:GaussianNetwork}}, qij, states, actions, μ_σ_old::Tuple)
    μ_old, σ_old = μ_σ_old
    μ, σ = RLCore.estimate_reward(p.actor, p.rng, states, is_sampling = false) #3D tensors with dimensions (action_size x 1 x batch_size)
...
function mpo_loss(p::MPOPolicy{<:Approximator{<:CategoricalNetwork}}, qij, states, actions, logits_old)
    logits = RLCore.estimate_reward(p.actor, p.rng, states, is_sampling = false) #3D tensors with dimensions (action_size x 1 x batch_size)
