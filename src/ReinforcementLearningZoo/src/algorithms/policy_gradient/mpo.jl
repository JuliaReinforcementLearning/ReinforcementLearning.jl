...
function MPOPolicy(;actor::Approximator, qnetwork1::Q, qnetwork2::Q, γ = 0.99f0, action_sample_size::Int, ϵ = 0.1f0, ϵμ = 1f-2, ϵΣ = 1f-4, α_scale = 1f0, αΣ_scale = 100f0, τ = 1f-3, max_grad_norm = 5f-1, rng = Random.default_rng()) where Q <: Approximator
    @assert RLCore.device(actor) == RLCore.device(qnetwork1) == RLCore.device(qnetwork2) "All network approximators must be on the same device"
    @assert RLCore.device(actor) == RLCore.device(rng) "The specified rng does not generate on the same device as the actor. Use `CUDA.CURAND.RNG()` to work with a CUDA GPU"
...
function update_critic!(p::MPOPolicy, batches)
    for (id, batch) in enumerate(batches)
        s, s′, a, r, t, = send_to_device(RLCore.device(p.qnetwork1), batch)
        γ, τ = p.γ, p.τ
...
function update_actor!(p::MPOPolicy, batches::Vector{<:NamedTuple{(:state,)}})
    states_batches = [send_to_device(RLCore.device(p.actor), reshape(batch[:state], size(batch[:state],1), 1, :)) for batch in batches] #vector of 3D tensors with dimensions (state_size x 1 x batch_size), sent to device
