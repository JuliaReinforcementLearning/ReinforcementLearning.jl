    ...
    @testset "hook: $(typeof(hook))" begin
        ...
        for mode in [:MultiAgent, :SingleAgent]
            for stage in stages
                ...
                if mode == :SingleAgent
                    RLCore.update!(hook_copy, stage, policy, env)
                elseif mode == :MultiAgent
                    RLCore.update!(hook_copy, stage, policy, env, :player_i)
                end
...
@testset "TotalRewardPerEpisode" begin
    ...
    for h in (h_1, h_2, h_3, h_4, h_5)
        RLCore.update!(h, PostActStage(), policy, env)
        @test h.reward == 1
        RLCore.update!(h, PostEpisodeStage(), policy, env)
        @test h.reward == 0
        @test h.rewards == [1]
        RLCore.update!(h, PostExperimentStage(), policy, env)
...
@testset "TimePerStep" begin
    ...
    for h in (h_1, h_2)
        RLCore.update!(h, PostActStage(), 1, 1)
        [(sleep(i); RLCore.update!(h, PostActStage(), 1, 1)) for i in sleep_vect]
...
@testset "StepsPerEpisode" begin
    ...
    [RLCore.update!(h, PostActStage()) for i in 1:100]

    @test h.count == 100

    RLCore.update!(h, PostEpisodeStage(), agent, env)
    @test h.count == 0
    @test h.steps == [100]

    RLCore.update!(h, PostExperimentStage(), agent, env)
    @test h.steps == [100, 0]
...
@testset "RewardsPerEpisode" begin
    ...
    for h in (h_1, h_2, h_3)
        RLCore.update!(h, PreEpisodeStage(), agent, env)
        @test h.rewards == [[]]

        env.pos = 1
        RLCore.update!(h, PostActStage(), agent, env)
        @test h.rewards == [[-1.0]]
        env.pos = 7
        RLCore.update!(h, PostActStage(), agent, env)
...
@testset "DoEveryNEpisode" begin
    ...
    for i in 1:3
        ...
        policy = RandomPolicy(legal_action_space(env))
        for t in 1:4
            RLCore.update!(h, stage, policy, env)
            @test env.pos == 1 + div(t,h.n)
        end     
