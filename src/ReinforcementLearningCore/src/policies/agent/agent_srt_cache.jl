...
function Base.push!(ts::Union{CircularArraySARSTTraces,ElasticArraySARTTraces}, xs::SA)
    push!(ts.traces[1].trace, xs.state)
    push!(ts.traces[2].trace, xs.action)
end

function Base.push!(ts::Union{CircularArraySARSTTraces,ElasticArraySARTTraces}, xs::SART)
    push!(ts.traces[1].trace, xs.state)
    push!(ts.traces[2].trace, xs.action)
