...
function RLBase.act!(env::BitFlippingEnv, action::Int)
    env.t += 1
    if 1 <= action <= env.N
...
RLBase.state(env::BitFlippingEnv, ::Observation{Any}, ::DefaultPlayer) = state(env::BitFlippingEnv, Observation{BitArray{1}}())
