...
RLBase.state(env::DefaultStateStyleEnv{S}, ::Observation, ::DefaultPlayer) where S = state(env.env, S)
RLBase.state(env::DefaultStateStyleEnv, ss::RLBase.AbstractStateStyle) = state(env.env, ss)
RLBase.state(env::DefaultStateStyleEnv{S}, player::AbstractPlayer) where S = state(env.env, S, player)
