...
struct MultiDiscreteSpace{T<:Integer, N} <:AbstractSpace
    ...
    high::Array{T, N}
    n::Int
    function MultiDiscreteSpace(high::Array{T, N}, low=ones(T, size(high))) where {T<:Integer, N}
        all(l < h for (l, h) in zip(low, high)) || throw(ArgumentError("each element of $high must be greater than $low"))
